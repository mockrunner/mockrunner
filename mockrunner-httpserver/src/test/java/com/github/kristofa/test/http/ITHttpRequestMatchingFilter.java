...
/**
 * Integration test that shows that chaining of {@link HttpRequestMatchingFilter
 * http request matching filters} works.
 * 
 * @author kristof
 */
public class ITHttpRequestMatchingFilter {
	...
	public void testChainedHttpRequestMatchingFilters() throws ClientProtocolException, IOException {
		...
		try {
			...
			assertEquals(receivedResponse.getFirstHeader("CONTENT_TYPE").getValue(), "type");
			assertEquals(receivedResponse.getFirstHeader("ACCEPT").getValue(), "accept");
		} finally {
	...
	/**
	 * Filter that removes an id header from the request but also updates the
	 * response to return the id header value that was received as input.
	 * 
	 * @author kristof
	 */
		...
		public HttpRequestMatchingContext filter(final HttpRequestMatchingContext context) {
			...
			if (!httpMessageHeaders.isEmpty()) {
				...
				if (!otherHeaders.isEmpty()) {
					...
					final HttpResponseImpl newResponse = new HttpResponseImpl(context.response().getHttpCode(),
							context.response().getContentType(), new String(otherId.toString()).getBytes(),
							context.response().getHttpMessageHeaders());
