...
/**
 * Is able to write a {@link HttpRequest} to file. A written request can be read
 * in again with {@link HttpRequestFileReaderImpl}.
 * 
 * @see HttpRequestFileReaderImpl
 * @author kristof
 */
public class HttpRequestFileWriterImpl implements HttpRequestFileWriter {
	...
	private void writeRequest(final HttpRequest request, final File requestFile) throws IOException {
		...
		final BufferedWriter writer = new BufferedWriter(
				new OutputStreamWriter(new FileOutputStream(requestFile), "UTF-8"));
