...
/**
 * {@link HttpResponseProvider} that is able to serve responses for
 * requests/responses previously saved by {@link HttpRequestResponseFileLogger}.
 * <p>
 * It supports submitting same request multiple times with different return
 * result.
 * <p>
 * It reads all http requests on construction and keeps them in memory. It does
 * not keep responses in memory. They are read from disk on request and not
 * cached.
 * 
...
public class FileHttpResponseProvider extends AbstractHttpResponseProvider {
	...
	/**
	 * Creates a new instance. Will try to find request/response files and will
	 * throw unchecked exception in case:
	 * <ul>
	 * <li>We can not find at least 1 request/response for given directory and
	 * file name.
	 ...
	 * @param fileName
	 *            Base file name. Should not contain extension. Will be suffixed
	 *            with sequence number and .txt extension. Should be same as
	 *            used in {@link HttpRequestResponseFileLogger}.
	 */
	...
	public FileHttpResponseProvider(final String directory, final String fileName, final HttpRequestMatcher matcher) {
		this(directory, fileName, new HttpRequestFileReaderImpl(), new HttpResponseFileReaderImpl(), matcher);
	}
	 ...
	 *            with sequence number and .txt extension. Should be same as
	 *            used in {@link HttpRequestResponseFileLogger}.
	 * @param requestFileReader
	 *            HTTP request file reader.
	 * @param responseFileReader
	 *            HTTP response file reader.
	 */
	public FileHttpResponseProvider(final String directory, final String fileName,
	...
	protected void lazyInitializeExpectedRequestsAndResponses() {
		...
		if (!requestFile.exists()) {
			throw new IllegalStateException(
					"No saved http request/responses found. File " + requestFile + " not found.");
		}
		...
		while (requestFile.exists()) {
			...
			if (!responseFile.exists()) {
				throw new IllegalStateException(
						"Found request file (" + requestFile + ") but no matching response file: " + responseFile);
			}
