 ...
 * Its purpose is to match simple requests that can be easily programmatically
 * defined in code. It supports matching:
 *
 * <ul>
 ...
 * </ul>
 * It does not support other http headers than Content-Type. If other http
 * header parameters are present in HttpRequest they will be ignored.
 * 
 * @see MockHttpServer
 * @author kristof
 */
public class SimpleHttpResponseProvider extends AbstractHttpResponseProvider {
	...
	public SimpleHttpResponseProvider(final HttpRequestMatcher matcher) {
		super(matcher);
		addHttpRequestMatchingFilter(new AllExceptContentTypeHeaderFilter());
	}
	 ...
	 *            Content type.
	 * @param requestEntity
	 *            Request entity as string.
	 * @return current {@link SimpleHttpResponseProvider}. Allows chaining
	 *         calls.
	 */
	public SimpleHttpResponseProvider expect(final Method method, final String path, final String contentType,
		...
		latestRequest.method(method).content(requestEntity.getBytes()).httpMessageHeader(CONTENT_TYPE_HTTP_HEADER_NAME,
				contentType);
	...
	public SimpleHttpResponseProvider expect(final Method method, final String path,
		...
		if (httpMessageHeaders != null) {
			latestRequest.addAllHttpMessageHeaders(httpMessageHeaders);
		}
	...
	public SimpleHttpResponseProvider expect(final Method method, final String path, final String contentType,
			final String requestEntity, final Set<HttpMessageHeader> httpMessageHeaders) {
		latestRequest = new HttpRequestImpl();
		if (httpMessageHeaders != null) {
			latestRequest.addAllHttpMessageHeaders(httpMessageHeaders);
		}
		...
		if (contentType != null) {
			latestRequest.httpMessageHeader(CONTENT_TYPE_HTTP_HEADER_NAME, contentType);
		}
		extractAndSetQueryParams(latestRequest, path);
		return this;
	}
	 ...
	 *         calls.
	 */
	public SimpleHttpResponseProvider respondWith(final int httpCode, final String contentType,
			final String requestEntity) {
	...
	public SimpleHttpResponseProvider respondWith(final int httpCode, final String contentType,
			final String requestEntity, final Set<HttpMessageHeader> httpMessageHeaders) {
		final HttpResponseImpl response = new HttpResponseImpl(httpCode, contentType,
				requestEntity == null ? null : requestEntity.getBytes(), httpMessageHeaders);
		addExpected(latestRequest, new DefaultHttpResponseProxy(response));
		return this;
	}
