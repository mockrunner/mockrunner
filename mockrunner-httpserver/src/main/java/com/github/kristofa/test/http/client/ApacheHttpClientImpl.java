...
/**
 * {@link HttpClient} implementation that uses
 * <a href="http://hc.apache.org/httpcomponents-client-ga/">Apache HTTP
...
public class ApacheHttpClientImpl implements HttpClient {
	...
	/* package */org.apache.http.client.HttpClient getClient() {
		// We use a Custom implementation because we don't want to modify the
		// requests/responses.
	...
	private HttpClientResponse<InputStream> execute(final HttpRequestBase request) throws IOException {
		...
		try {
			...
			client.getConnectionManager().shutdown(); // In case of exception we
														// should close
														// connection manager
														// here.
			throw e;
	...
	private HttpClientResponse<InputStream> buildResponse(final org.apache.http.client.HttpClient client,
		...
		Set<HttpMessageHeader> messageHeader = new HashSet<HttpMessageHeader>();
		for (Header aHeader : allHeaders) {
			HttpMessageHeader tempHeader = new HttpMessageHeader(aHeader.getName(), aHeader.getValue());
			messageHeader.add(tempHeader);
		}
		final ApacheHttpClientResponseImpl<InputStream> httpResponse = new ApacheHttpClientResponseImpl<InputStream>(
				status, client, messageHeader);
