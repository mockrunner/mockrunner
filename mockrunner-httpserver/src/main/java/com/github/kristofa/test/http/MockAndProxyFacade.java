...
/**
 * Facade that lets you easily configure and use either the
 * {@link LoggingHttpProxy} for using real services and logging
 * requests/responses or the {@link MockHttpServer} for running your code
 * against mocked http requests/responses. You can use {@link MockHttpServer} or
 * {@link LoggingHttpProxy} by themselves but if you use both it is most likely
 * that this class will make things easier for you.
 * 
 * @author kristof
 */
public class MockAndProxyFacade {
	...
	/**
	 * Specifies the mode in which {@link MockAndProxyFacade} should be
	 * operating.
	 * 
	 * @author kristof
	 */
	...
	public static class Builder {
		 ...
		 * 
		 * @param mode
		 *            Operation mode. Should not be <code>null</code>.
		 * @return Builder.
		 */
	...
	private MockAndProxyFacade(final Builder builder) {
		...
		mode = builder.mode;
		// Validate.isTrue(builder.port > 0, "You should have set port in
		// builder!");
	...
	/**
	 * Starts either {@link MockHttpServer} or {@link LoggingHttpProxy}
	...
	/**
	 * Stops either {@link MockHttpServer} or {@link LoggingHttpProxy} depending
	 * on operation mode.
	 * 
	 * @throws IOException
	 *             In case stopping fails.
	 */
