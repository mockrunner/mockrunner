...
/**
 * Http proxy that supports logging requests/reponses. Its purpose is to be a
 * 'man in the middle' which can be used to capture request/responses that can
 * be mocked later on for testing purposes. It forwards requests it gets to
 * another service and returns the result of that service unmodified to the
 * requester. While doing that it also allows logging request/response pairs.
 *
 * Those persisted request/response pairs can be mocked by
 * {@link MockHttpServer}.
 *
 * Using the {@link LoggingHttpProxy} to persist request/responses and using
 * them with the {@link MockHttpServer} is especially useful for complex
 * responses that are not that easy to mock by hand. It allows building a
 * <a href=
 * "http://googletesting.blogspot.be/2012/10/hermetic-servers.html">hermetic
 * server</a>.
 *
 * The {@link LoggingHttpProxy} will return following HTTP return codes when
 * things go wrong:
 * <ul>
 * <li>570: We could not build a forward request for input request. Missing of
 * faulty {@link ForwardHttpRequestBuilder}.
...
public class LoggingHttpProxy {
	...
	public int start() throws IOException {
		...
		LOGGER.debug("Started on port: " + connectedPort);
