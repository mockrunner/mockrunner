...
/**
 * Builds a HttpRequest for which the content is stored on disk. It can
 * reconstruct a HttpRequest which was previously stored with
 * {@link HttpRequestFileWriterImpl}.
 * 
...
public class HttpRequestFileReaderImpl implements HttpRequestFileReader {
	...
	private void readRequest(final HttpRequestImpl request, final File requestFile, final File requestEntityFile)
		...
		final BufferedReader reader = new BufferedReader(
				new InputStreamReader(new FileInputStream(requestFile), "UTF-8"));
		try {
			...
			if (!PATH.equals(nextSection)) {
				throw new IllegalStateException(
						"Expected " + PATH + " after " + HTTP_MESSAGE_HEADER + " but got " + nextSection);
			}
			...
			if (nextSection2 != null) {
				throw new IllegalStateException(
						"Expected nothing after " + QUERY_PARAMETERS + " but got " + nextSection2);
			}
